...
- song에 언어 컬럼을 만들지 마라. 곡의 언어는 song_language다 (D-051).
  original_language_id는 만들지 마라 — 원제 언어 개념 자체가 없다 (D-072).
...
```
[작업]
- 곡 만들기 · 조회 · 수정: 이름(언어별 + 대표), 곡 언어, 프로듀서 크레딧(role 자동), 정답 패턴, status
- 이름 검증: 이름이 있는 언어마다 대표 1개, 이름은 1개 이상 있어야 한다. 어기면 400 (D-061, D-072)
- GET /api/v1/admin/languages
- 프로듀서 자동완성 + 새로 만들기. 이름은 언어별 (D-073), UNIQUE는 (language_id, name) — 같은 언어에서 같은 이름이면 409
...
[완료]
1) 이름 없이 곡을 만들면 400, 한 언어에 대표가 둘이어도 400
...
4) 없는 프로듀서를 새로 만들 수 있고, 같은 언어에서 같은 이름으로 다시 만들면 409
