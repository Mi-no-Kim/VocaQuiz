...
모든 `UNIQUE` 제약의 이름은 `uk_`로 시작한다 (`uk_producer_name`처럼). 관례가 아니라 계약이다 —
`GlobalExceptionHandler`가 사전검사(예: `findByName` 후 `save`)와 실제 저장 사이의 레이스로
DB가 대신 막아 준 UNIQUE 위반을 409로 바꿔 줄 때, 제약 이름에 `uk_`가 있는지로 "진짜 충돌"과
NOT NULL·FK 같은 다른 무결성 위반(버그)을 구분한다. `uk_`가 아닌 이름으로 UNIQUE를 만들면
그 위반은 이 구분을 못 타고 500으로 새 나간다.
